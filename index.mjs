...
// Een scheldwoord middenin de zin vervangen door een los woord levert bijna nooit een
// kloppende zin op (bv. "fuck you" -> "haat you"). Daarom laten we de AI het HELE
// bericht herschrijven: dezelfde taal/betekenis, maar elk scheldwoord vervangen door
// (een vorm van) het woord "haat" — het enige woord dat voor grove taal gebruikt mag
// worden. Dit hieronder is alleen de DETECTIE; het herschrijven zelf gebeurt in
// rewriteMessageWithAi() (verderop, want die heeft de Groq-aanroep-functies nodig).
const FILTER_REPLACEMENT_MESSAGE =
  (process.env.FILTER_REPLACEMENT_MESSAGE || "Ik haat je.").trim() || "Ik haat je.";
...
const CHANGELOG_ENTRIES = [
  ...
  `🆕 Berichten met scheldwoorden worden nu automatisch onzichtbaar vervangen door "${FILTER_REPLACEMENT_MESSAGE}" (net als een Roblox-chatfilter)`,
...
// De AI herschrijft het volledige bericht: zelfde taal/betekenis/toon, maar elk
// scheldwoord of grove uitdrukking wordt vervangen door (een vorm van) het woord
// "haat" — dat is het ENIGE woord dat voor grove taal gebruikt mag worden. Dit levert
// altijd een kloppende, natuurlijke zin op, in plaats van een kapotte woord-voor-woord
// vervanging zoals "fuck you" -> "haat you".
const FILTER_REWRITE_SYSTEM_PROMPT = `
Je herschrijft een Discord-bericht dat grove/ongepaste taal bevat naar een nette, natuurlijke zin.

Regels, volg deze STRIKT:
- Herschrijf het VOLLEDIGE bericht opnieuw als een lopende zin — nooit maar één los woord als antwoord.
- Vervang elk scheldwoord of belediging door het woord "haat", natuurlijk ingepast in de zin (bijvoorbeeld "haat", "ik haat je", "ik haat dit"). Dit is het ENIGE woord dat je als vervanging voor grove taal mag gebruiken — verzin geen andere eufemismen zoals "boos" of "vervelend".
- De rest van het bericht (de niet-grove inhoud, de betekenis, de context, de taal waarin het geschreven is) blijft zoveel mogelijk hetzelfde staan.
- Schrijf in dezelfde taal als het origineel (Nederlands blijft Nederlands, Engels blijft Engels, enzovoort).
- Antwoord ALLEEN met de herschreven zin zelf: geen aanhalingstekens, geen uitleg, geen emoji's, geen extra zinnen erbij.
- Als er na het weghalen van de grove taal niets zinnigs overblijft, antwoord dan met exact: Ik haat je.

Voorbeelden (alleen ter illustratie van het gewenste formaat, niet om letterlijk te kopiëren):
Bericht: "fuck you"
Herschreven: Ik haat je.

Bericht: "this game is shit"
Herschreven: Ik haat dit spel.

Bericht: "kanker weer vandaag zeg"
Herschreven: Ik haat dit weer vandaag zeg.

Bericht: "shut up you fucking idiot"
Herschreven: Hou je mond, ik haat dit.
`.trim();
...
async function rewriteMessageWithAi(originalContent) {
  try {
    const rewritten = await askGroqRaw(FILTER_REWRITE_SYSTEM_PROMPT, originalContent, {
      maxTokens: 150,
      temperature: 0.3, // laag houden voor consistente, voorspelbare herschrijvingen
    });
    const cleaned = rewritten.trim().replace(/^["'“”]+|["'“”]+$/g, "").trim();
    const wordCount = cleaned.split(/\s+/).filter(Boolean).length;
    // Extra veiligheidsnet: als de AI leeg, te kort (bv. alleen "Haat.") of nog steeds
    // een verboden woord teruggeeft, val terug op het vaste bericht.
...
function buildConfigMessage() {
  ...
  return [
    ...
    `• Filter-vervangbericht: "${FILTER_REPLACEMENT_MESSAGE}"`,
...
client.once("clientReady", async () => {
  ...
  console.log(`🚫 Woordfilter: ${BANNED_WORD_LIST.length} woord(en) ingesteld (met leetspeak/uitrek/uit-elkaar-detectie), vervangbericht: "${FILTER_REPLACEMENT_MESSAGE}"`);
...
client.on("messageCreate", async (message) => {
  ...
  // vermomd als dezelfde gebruiker, met een door de AI herschreven versie waarin elk
  // scheldwoord vervangen is door (een vorm van) "haat".
