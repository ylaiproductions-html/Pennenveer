...
Je herschrijft een Discord-bericht dat grove/ongepaste taal bevat, zodat het netjes leesbaar wordt.

Regels, volg deze STRIKT:
- Vervang elk scheldwoord, belediging of grove uitdrukking door het woord "haat" (eventueel vervoegd/ingepast in de zin, bijvoorbeeld "ik haat je", "ik haat dit"). Dit is het ENIGE woord dat je mag gebruiken als vervanging voor grove taal — verzin geen andere eufemismen.
- De rest van het bericht (de niet-grove inhoud, de betekenis, de taal waarin het geschreven is) blijft zoveel mogelijk hetzelfde staan.
- Het resultaat moet één kloppende, natuurlijke zin (of kort berichtje) zijn, in dezelfde taal als het origineel.
- Antwoord ALLEEN met het herschreven bericht zelf: geen aanhalingstekens, geen uitleg, geen emoji's, geen extra zinnen erbij.
...
async function rewriteMessageWithAi(originalContent) {
  try {
    const rewritten = await askGroqRaw(FILTER_REWRITE_SYSTEM_PROMPT, originalContent, { maxTokens: 150 });
    const cleaned = rewritten.trim().replace(/^["'“”]+|["'“”]+$/g, "").trim();
    // Extra veiligheidsnet: als de AI toch nog een verboden woord laat staan
    // (bv. bij een storing of rare output), val terug op het vaste bericht.
    if (!cleaned || detectBannedWords(cleaned)) return FILTER_REPLACEMENT_MESSAGE;
    return cleaned;
  } catch (err) {
  ...
  if (isFeatureOn("woordfilter") && detectBannedWords(message.content)) {
    ...
    try {
      if (deleted) {
        ...
        await sendAsImpersonatedAuthor(message.channel, message, rewritten);
