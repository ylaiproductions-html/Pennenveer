...
// ---------- Woordfilter: bericht ONZICHTBAAR VERVANGEN (Roblox-stijl) ----------
// Een scheldwoord ergens middenin de zin vervangen door een los woord levert bijna
// nooit een kloppende zin op (bv. "fuck you" -> "haat you"). Daarom vervangen we bij
// detectie het HELE bericht door één vaste, grammaticaal correcte zin met "haat" erin
// — net zoals veel chatfilters een bericht met scheldwoorden volledig herschrijven.
...
// uitgerekt/uit-elkaar-getrokken varianten). Bij een treffer wordt niet het losse
// woord vervangen, maar het HELE bericht door FILTER_REPLACEMENT_MESSAGE — zo blijft
// de uitkomst altijd een nette, kloppende zin i.p.v. een kapotte halve zin.
function filterBannedWords(text) {
  if (!BANNED_WORD_LIST.length) return { filtered: text, changed: false };

  const found = fuzzyBannedWordRegexes.some((regex) => {
    regex.lastIndex = 0;
    return regex.test(text);
  });

  if (!found) return { filtered: text, changed: false };
  return { filtered: FILTER_REPLACEMENT_MESSAGE, changed: true };
}
...
client.on("messageCreate", async (message) => {
  ...
  // ----- Woordfilter: scheldwoorden worden onzichtbaar vervangen (Roblox-stijl) -----
  // In plaats van alleen te waarschuwen: het bericht wordt verwijderd en direct
  // teruggeplaatst als "vermomd" bericht van dezelfde gebruiker, maar dan met elk
  // hele bericht vervangen door FILTER_REPLACEMENT_MESSAGE (bv. "Ik haat je."),
  // zodat het resultaat altijd een kloppende zin is.
  if (isFeatureOn("woordfilter")) {
    const { filtered, changed } = filterBannedWords(message.content);
    if (changed) {
      console.log(`🚫 Verboden taal gefilterd bij ${message.author.username} in ${message.channelId}.`);
      ...
      try {
        if (deleted) {
          ...
          await sendAsImpersonatedAuthor(message.channel, message, filtered);
        } else {
        ...
        await logToModChannel(
          `🚫 **${message.author.username}** (<@${message.author.id}>) gebruikte verboden taal in <#${message.channelId}> — ${
            deleted ? "het bericht is automatisch aangepast" : "kon niet worden aangepast (rechten ontbreken)"
          }. (${progressLabel(countInWindow)})\n**Origineel:** ${message.content.slice(0, 500)}\n**Aangepast:** ${filtered.slice(0, 500)}`
        );
